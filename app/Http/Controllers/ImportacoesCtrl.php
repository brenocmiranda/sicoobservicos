...
class ImportacoesCtrl extends Controller
{
	...
	public function ImportarManual(Request $request){
		...
		if ($request->hasFile('myData') && $request->file('myData')->isValid()){
			...
			// pro_sipag
			if($request->relatorio == 'pro_sipag'){
				try{
					$nameFile = 'pro_sipag-'.date('dmYHis').'.'.request()->file('myData')->getClientOriginalExtension();
					$upload = $request->myData->storeAs('importacoes', $nameFile);
					Excel::queueImport(new pro_sipag, getcwd().'/storage/app/importacoes/'.$nameFile)->onQueue('low');
					return response()->json(['status' => true]);
				} catch (\Exception $ex){
		...
		if(count(scandir("outlook")) > 2){
			...
			if(file_exists(getcwd().'/outlook/pro_sipag.xlsx')){
				try{
					...
					(new pro_sipag)->queue(getcwd().'/storage/app/importacoes/'.$nameFile)->onQueue('low')->delay(now()->addMinutes(7));
				} catch (\Exception $ex){
	...
	public function DonwloadRelatorios(){
		...
		// Baixando todos relatórios da pasta
