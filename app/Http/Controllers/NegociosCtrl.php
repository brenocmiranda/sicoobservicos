...
class NegociosCtrl extends Controller
{
	...
	public function DatatablesAnalise(){
		$dados = Associados::join('cca_contacapital', 'cli_id_associado', 'cli_associados.id')->where('sigla', 'PF')->where('situacao_capital', 'ATIVO')->select('cli_associados.id', 'nome', 'documento', 'renda', 'nome_gerente')->orderBy('nome', 'ASC')->get();
