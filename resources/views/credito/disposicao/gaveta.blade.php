...
<div class="container-fluid">
	<div class="row bg-title">
		<div class="col-lg-3 col-md-4 col-sm-4 col-xs-12">
			<h4 class="page-title">{{$arm->referencia}}</h4> 
		</div>
		<div class="col-lg-9 col-sm-8 col-md-8 col-xs-12">
			<ol class="breadcrumb">
				<li><a href="{{route('dashboard.credito')}}">Crédito</a></li>
				<li><a href="{{ route('exibir.disposicao.credito') }}">Disposição</a></li>
				<li><a class="active">Gaveta</a></li>
			</ol>
		</div>
	</div>
	<div class="card">
...
<script type="text/javascript">	
	...
	$(document).ready( function(){
		...
		$.ajax({
			url: '{{ route("listar.gaveta.credito", $arm->id) }}',
			type: 'GET',
			success: function(table){
				...
				$('#table tbody').on('click', 'button#editar', function (){
					...
					$.get("{{url('app/credito/contratos/garantias')}}/"+data.id, function(dataGarantias){
		...
		$('#modal-editar #formEditar').on('submit', function(e){
			...
			var data = table.row('tr.active').data();
			$.ajax({
				url: "{{url('app/credito/contratos/editar')}}/"+data.id,
				type: 'POST',
		...
		$('#modal-alterar #formAlterar').on('submit', function(e){
			...
			e.preventDefault();
			$.ajax({
				url: "{{url('app/credito/contratos/alterar')}}/"+data.id,
				type: 'POST',
