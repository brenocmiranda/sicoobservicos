			...
			<div class="h-100 row col">
				<div class="col-lg-12 position-absolute">
					<div class="row mx-auto">
						<button class="btn btn-primary btn-outline ml-auto" id="adicionar" name="adicionar" title="Adicionar nova finalidade" data-toggle="modal" data-target="#modal-adicionar" style="z-index: 10">
							<i class="m-0 pr-1 mdi mdi-plus"></i> 
							<span>Nova finalidade</span> 
						</button>
					</div>
				</div>
			</div>
...
@section('modal')
	@include('credito.configuracoes.finalidades.adicionar')
	@include('credito.configuracoes.finalidades.editar')
	@include('credito.configuracoes.finalidades.detalhes')
@endsection
...
<script type="text/javascript">
	$(document).ready( function (){
		...
		$('#table').DataTable({
			...
			searching: true,
			destroy: true,
			ajax: "{{ route('listar.finalidades.credito') }}",
			serverSide: true,
			"columns": [ 
			{ "data": "id","name":"id"},
		...
		$('#table tbody').on('click', 'button#alterar', function () {
			...
			$(this).parent('tr').addClass('selected');
			var data = table.row('tr.selected').data();
			var url = "{{url('app/gestao/credito/finalidades/alterar')}}/"+data.id;
			swal({
		...
		$('#modal-adicionar #formAdicionar').on('submit', function(e){
			...
			e.preventDefault();
			$.ajax({
				url: '{{ route("adicionar.finalidades.credito") }}',
				type: 'POST',
		...
		$('#modal-editar #formEditar').on('submit', function(e){
			...
			e.preventDefault();
			$.ajax({
				url: 'finalidades/editar/'+data.id,
				type: 'POST',
