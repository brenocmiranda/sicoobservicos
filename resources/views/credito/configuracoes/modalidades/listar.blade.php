			...
			<div class="h-100 row col">
				<div class="col-lg-12 position-absolute">
					...
					<div class="row mx-auto">
						<button class="btn btn-primary btn-outline ml-auto" id="adicionar" name="adicionar" title="Adicionar nova modalidade" data-toggle="modal" data-target="#modal-adicionar" style="z-index: 10">
...
@section('modal')
	@include('credito.configuracoes.modalidades.adicionar')
	@include('credito.configuracoes.modalidades.editar')
	@include('credito.configuracoes.modalidades.detalhes')
@endsection
...
<script type="text/javascript">
	$(document).ready( function (){
		...
		$('#table').DataTable({
			...
			searching: true,
			destroy: true,
			ajax: "{{ route('listar.modalidades.credito') }}",
			serverSide: true,
			"columns": [ 
			{ "data": "id","name":"id"},
		...
		$('#table tbody').on('click', 'button#alterar', function () {
			...
			$(this).parent('tr').addClass('selected');
			var data = table.row('tr.selected').data();
			var url = "{{url('app/gestao/credito/modalidades/alterar')}}/"+data.id;
			swal({
		...
		$('#modal-adicionar #formAdicionar').on('submit', function(e){
			...
			e.preventDefault();
			$.ajax({
				url: '{{ route("adicionar.modalidades.credito") }}',
				type: 'POST',
		...
		$('#modal-editar #formEditar').on('submit', function(e){
			...
			e.preventDefault();
			$.ajax({
				url: 'modalidades/editar/'+data.id,
				type: 'POST',
