...
@section('modal')
	@include('tecnologia.configuracoes.inventario.equipamentos.adicionar')
	@include('tecnologia.configuracoes.inventario.equipamentos.editar')
	@include('tecnologia.configuracoes.inventario.equipamentos.detalhes')
@endsection
...
<script type="text/javascript">
	$(document).ready( function (){
		...
		$('#table').DataTable({
			...
			searching: true,
			destroy: true,
			ajax: "{{ route('listar.equipamentos.inventario') }}",
			serverSide: true,
			"columns": [ 
		...
		$('#table tbody').on('click', 'button#alterar', function () {
			...
			$(this).parent('tr').addClass('selected');
			var data = table.row('tr.selected').data();
			var url = "{{url('app/gti/configuracoes/equipamentos/alterar')}}/"+data.id;
			swal({
		...
		$('#modal-adicionar #formAdicionar').on('submit', function(e){
			...
			e.preventDefault();
			$.ajax({
				url: '{{ route("adicionar.equipamentos.inventario") }}',
				type: 'POST',
		...
		$('#modal-editar #formEditar').on('submit', function(e){
			...
			e.preventDefault();
			$.ajax({
				url: 'equipamentos/editar/'+data.id,
				type: 'POST',
